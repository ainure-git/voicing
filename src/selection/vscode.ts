...
export function createSelectionDeps(copyCommand: string): SelectionDeps {
  return {
    ...
    copySelection: async () => {
      // The terminal copy command is gated on `terminalFocus`. It works when
      // invoked while the terminal keeps focus (right-click "Leer selección" in
      // the terminal, a keybinding, or the command palette right after using the
      // terminal). We deliberately do NOT re-focus the terminal here: a
      // programmatic focus can clear the xterm selection and would break the
      // reliable right-click path. From a status-bar click the terminal has
      // already lost focus (and xterm cleared the selection), so this is a
      // no-op and the caller offers "Leer portapapeles" as the fallback.
